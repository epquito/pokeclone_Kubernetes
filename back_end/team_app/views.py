...
class TeamManager(UserPermissions):
    ...
    def get(self, request):
        ...
        for team in user_teams:
            ...
            team_data["pokemons"] = TeamPokemonSerializer(
                team.team_pokemons.all(), many=True
            ).data
        ...
        try:
            if action == "pick":
                ...
                for pokemon_id in pokemon_ids:
                    user_pokemon = get_object_or_404(
                        UserPokemon, pokemon_id=pokemon_id, user=request.user
                    )
                    position = TeamPokemon.objects.filter(team=team).count() + 1
                    TeamPokemon.objects.create(
                        team=team, user_pokemon=user_pokemon, position=position
                    )
                ...
                for pokemon_id in pokemon_ids:
                    ...
                    team_pokemon = TeamPokemon.objects.get(
                        team=team, user_pokemon=user_pokemon
                    )
                    team_pokemon.delete()
